...
from config import aaMovementAmp, useMask, maskHeight, maskWidth, aaQuitKey, confidence, headshot_mode, cpsDisplay, visuals, centerOfScreen, screenShotWidth, maskSide
...
def main():
    ...
    model = DetectMultiBackend('best123.engine', device=torch.device(
    ...
    with torch.no_grad():
        while win32api.GetAsyncKeyState(ord(aaQuitKey)) == 0:
            ...
            im = np.array([npImg], dtype=np.float16) / 255
            ...
            if len(targets) > 0:
                if (centerOfScreen):
                    ...
                    targets = targets.sort_values("dist_from_center").reset_index(drop=True)
                ...
                if last_mid_coord:
                    ...
                    targets = targets.sort_values(by="dist", ascending=False).reset_index(drop=True)
                ...
                key2 = win32api.GetAsyncKeyState(0x2)
                if key2:
            ...
            if visuals:
                ...
                for i in range(0, len(targets)):
                    ...
                    midX = targets.iloc[i]['current_mid_x']
                    midY = targets.iloc[i]['current_mid_y']
            ...
            # Cap loop to TARGET_FPS to avoid saturating CPU/GPU and freezing Windows input
            now = time.time()
